...
; Positive cases: the real PipelineStartEP shape of the TWO 2mm launch sites.
; 2mm launches two GEMM kernels back-to-back; each `parallel_for_lambda_impl`
; instantiation's mangled name embeds its kernel tag class (15Polybench_2mm_1
; / 15Polybench_2mm_2). The by-value range arguments are stored into the
; %UserRange alloca; the getRoundedRange call's two i64 args are loads from a
; temporary copy %agg.tmp11 that is filled by a memcpy FROM %UserRange. The
; pass must rewrite BOTH functions' stores to ceil(V/16)*16 =
; ((V+15) udiv 16) * 16 (the FULL padded grid, one work-item per output cell
; of the sycl-2mm-local-tile device pass; divisibility by 16 satisfies the
; UR launch validator for the SYCL_FORCE_LOCAL_SIZE local size (16,16)).

%class.sycl.handler = type { i32 }
%"class.sycl::_V1::range" = type { i64, i64 }

@.str.87 = private unnamed_addr constant [22 x i8] c"_ZTS15Polybench_2mm_1\00", align 1
@.str.93 = private unnamed_addr constant [22 x i8] c"_ZTS15Polybench_2mm_2\00", align 1
...
define void @_ZN4sycl3_V17handler24parallel_for_lambda_implI15Polybench_2mm_2E(ptr %cgh, i64 %UserRange.coerce0, i64 %UserRange.coerce1) {
entry:
; CHECK-LABEL: @_ZN4sycl3_V17handler24parallel_for_lambda_implI15Polybench_2mm_2E(
  %UserRange = alloca %"class.sycl::_V1::range", align 8
  %agg.tmp11 = alloca %"class.sycl::_V1::range", align 8
  %ret = alloca %"class.sycl::_V1::range", align 8
...
; kernel (not one of the two 2mm tag classes) -> per-function name filter
; leaves it untouched, even though the module carries the 2mm anchors.
