; RUN: opt < %s -passes=sycl-rewrite-gemm-range -S | FileCheck %s

; Positive case: the real PipelineStartEP shape of the gemm launch site.
; The `parallel_for_lambda_impl` instantiation's mangled name embeds the
; kernel tag class (4Gemm -- the mangled template argument `Gemm`). The
; by-value range arguments are stored into the %UserRange alloca; the
; getRoundedRange call's two i64 args are loads from a temporary copy
; %agg.tmp11 that is filled by a memcpy FROM %UserRange. The pass must
; rewrite the stores to ceil(V/16)*16 = ((V+15) udiv 16) * 16 (the FULL
; padded grid, one work-item per output cell of the sycl-gemm-local-tile
; device pass; divisibility by 16 satisfies the UR launch validator for the
; SYCL_FORCE_LOCAL_SIZE local size (16,16)).

%class.sycl.handler = type { i32 }
%"class.sycl::_V1::range" = type { i64, i64 }

@.str.87 = private unnamed_addr constant [10 x i8] c"_ZTS4Gemm\00", align 1
...
define void @_ZN4sycl3_V17handler24parallel_for_lambda_implI4GemmE(ptr %cgh, i64 %UserRange.coerce0, i64 %UserRange.coerce1) {
entry:
; CHECK-LABEL: @_ZN4sycl3_V17handler24parallel_for_lambda_implI4GemmE(
...
; kernel (not the gemm tag class) -> per-function name filter leaves it
; untouched, even though the module carries the gemm anchor.
; NOTE: `_ZTS16Polybench_Gemm1` (the gemm_opt source-level kernel) CONTAINS
; "Gemm" -- the I4Gemm fragment match must reject it (an EXACT tag
; distinction; a bare contains("Gemm") would misfire here).
define void @_ZN4sycl3_V17handler24parallel_for_lambda_implI16Polybench_Gemm1E(ptr %cgh, i64 %UserRange.coerce0, i64 %UserRange.coerce1) {
entry:
; CHECK-LABEL: @_ZN4sycl3_V17handler24parallel_for_lambda_implI16Polybench_Gemm1E(
  %UserRange = alloca %"class.sycl::_V1::range", align 8
  %agg.tmp11 = alloca %"class.sycl::_V1::range", align 8
  %ret = alloca %"class.sycl::_V1::range", align 8
