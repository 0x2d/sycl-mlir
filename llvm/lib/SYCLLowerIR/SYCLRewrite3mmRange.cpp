//===- SYCLRewrite3mmRange.cpp - 3mm launch-range rewrite ------*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// See SYCLRewrite3mmRange.h for the design. Summary of the IR shape matched
// per launch site (from a -O3 -Xclang -disable-llvm-passes -emit-llvm dump of
// the 3mm.cpp host TU at PipelineStartEP; identical to the 2mm shape):
//
//   define ... @...parallel_for_lambda_implI15Polybench_3mm_N...(
...
//
// There are THREE such instantiations (one per 3mm kernel, each identified
// by its kernel tag class in the mangled name); the pass rewrites the
// UserRange stores in ALL THREE.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "sycl-rewrite-3mm-range"

using namespace llvm;

STATISTIC(NumLaunchSitesRewritten, "Number of launch sites rewritten");

static constexpr StringRef THREE_MM_KERNEL_1_NAME = "_ZTS15Polybench_3mm_1";
static constexpr StringRef THREE_MM_KERNEL_2_NAME = "_ZTS15Polybench_3mm_2";
static constexpr StringRef THREE_MM_KERNEL_3_NAME = "_ZTS15Polybench_3mm_3";
/// The kernel tag class fragments embedded in each
/// `parallel_for_lambda_impl` instantiation's mangled name.
static constexpr StringRef THREE_MM_KERNEL_1_FRAGMENT = "Polybench_3mm_1";
static constexpr StringRef THREE_MM_KERNEL_2_FRAGMENT = "Polybench_3mm_2";
static constexpr StringRef THREE_MM_KERNEL_3_FRAGMENT = "Polybench_3mm_3";
...
/// `__builtin_sycl_unique_stable_name(Polybench_3mm_N)` is a private constant
/// whose content is exactly "_ZTS15Polybench_3mm_N".
...
/// Does \p F name one of the three 3mm `parallel_for_lambda_impl`
/// instantiations? Each instantiation's mangled name embeds its kernel tag
/// class (`15Polybench_3mm_1` / `15Polybench_3mm_2` / `15Polybench_3mm_3`).
static bool is3mmLambdaImpl(const Function &F) {
  StringRef Name = F.getName();
  if (!Name.contains(LAMBDA_IMPL_FRAGMENT))
    return false;
  return Name.contains(THREE_MM_KERNEL_1_FRAGMENT) ||
         Name.contains(THREE_MM_KERNEL_2_FRAGMENT) ||
         Name.contains(THREE_MM_KERNEL_3_FRAGMENT);
}

PreservedAnalyses SYCLRewrite3mmRangePass::run(Module &M,
                                               ModuleAnalysisManager &) {
  // Kernel-name anchor: without it this TU launches none of the 3mm kernels.
  if (!findStringAnchor(M, THREE_MM_KERNEL_1_NAME) &&
      !findStringAnchor(M, THREE_MM_KERNEL_2_NAME) &&
      !findStringAnchor(M, THREE_MM_KERNEL_3_NAME)) {
    LLVM_DEBUG(dbgs() << "sycl-rewrite-3mm-range: no 3mm kernel-name anchor, "
                         "no-op\n");
  ...
  bool Changed = false;
  for (Function &F : M) {
    if (F.isDeclaration() || !is3mmLambdaImpl(F))
      continue;
    ...
    // built at the store's insertion point. (In 3mm this is exactly the two
    ...
    for (StoreInst *ST : Stores) {
      ...
      IRBuilder<> B(ST);
      Value *PlusTile =
          B.CreateAdd(V, ConstantInt::get(VTy, TILE - 1), "3mm.plus");
      Value *Ceil =
          B.CreateUDiv(PlusTile, ConstantInt::get(VTy, TILE), "3mm.ceil");
      Value *Padded =
          B.CreateMul(Ceil, ConstantInt::get(VTy, TILE), "3mm.padded");
    ...
    if (RewrittenInF) {
      ...
      Changed = true;
      LLVM_DEBUG(dbgs() << "sycl-rewrite-3mm-range: rewrote " << RewrittenInF
