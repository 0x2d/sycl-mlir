//===- SYCLRewriteSyr2kRange.cpp - SYR2K launch-range rewrite ---*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// See SYCLRewriteSyr2kRange.h for the design. Summary of the IR shape matched
// (from a -O3 -emit-llvm dump of the syr2k.cpp host TU at PipelineStartEP):
//
...
#define DEBUG_TYPE "sycl-rewrite-syr2k-range"

using namespace llvm;

STATISTIC(NumLaunchSitesRewritten, "Number of launch sites rewritten");

static constexpr StringRef SYR2K_KERNEL_NAME = "_ZTS6Syr2k1";
...
/// `__builtin_sycl_unique_stable_name(Syr2k1)` is a private constant whose
/// content is exactly "_ZTS6Syr2k1".
...
PreservedAnalyses SYCLRewriteSyr2kRangePass::run(Module &M,
                                                 ModuleAnalysisManager &) {
  // Kernel-name anchor: without it this TU does not launch the syr2k kernel.
  if (!findStringAnchor(M, SYR2K_KERNEL_NAME)) {
    LLVM_DEBUG(dbgs() << "sycl-rewrite-syr2k-range: no " << SYR2K_KERNEL_NAME
                      << " anchor, no-op\n");
  ...
  for (Function &F : M) {
    ...
    // Resolve the ORIGIN range object. The call's loads may read a temporary
    // copy (`%agg.tmp11`) that is filled by a memcpy FROM the real range
    // object (`%UserRange`, itself filled by the two i64 stores of the
    // by-value range arguments of parallel_for_lambda_impl). Rewrite the
    // stores at the origin so every downstream consumer (getRoundedRange
    // args, the range-rounding wrapper's UserRange member, checkValueRange,
    // MNDRDesc.set) sees ceil(N/8).
    ...
    // at the store's insertion point. (In syr2k this is exactly the two
    // stores of N into %UserRange.)
    ...
    for (StoreInst *ST : Stores) {
      ...
      IRBuilder<> B(ST);
      Value *PlusTile =
          B.CreateAdd(V, ConstantInt::get(VTy, TILE - 1), "syr2k.plus");
      Value *Ceil =
          B.CreateUDiv(PlusTile, ConstantInt::get(VTy, TILE), "syr2k.ceil");
    ...
    if (RewrittenInF) {
      ...
      Changed = true;
      LLVM_DEBUG(dbgs() << "sycl-rewrite-syr2k-range: rewrote " << RewrittenInF
