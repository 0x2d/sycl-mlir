//===- ThreeMmLocalTile.cpp - 3mm 16x16 local-memory (LDS) tile rewrite --*- C++ -*-===//
//
...
//===----------------------------------------------------------------------===//
//
// Device-side half of the 3MM optimization, transcribing the LDS-tiled
// micro-kernel of the source-level `polybench/orise/3mm_opt.cpp` (a direct
...
// measured 2.2x SLOWER and was replaced). ONE instance of this pass rewrites
// ALL THREE 3mm kernels (`Polybench_3mm_1` E += A*B, `Polybench_3mm_2`
// F += C*D, `Polybench_3mm_3` G += E*F); they share the GEMM shape
// out[i,j] = seed(i,j) + sum_k in1[i,k]*in2[k,j] and the captured-struct
// layout (i64 size_, OUT accessor, IN1 read, IN2 read) and -- unlike 2mm,
// whose kernel 2 wrote from zero -- ALL THREE seed from the existing output
// (SeedFromC): every 3mm kernel's output accessor is read_write and every
// baseline body is `X[item] += ...`, so loading the existing OUT value is
// the semantically faithful seed for all three (first run the buffers are
// zero from std::vector::resize; repeat runs replicate the baseline).
//
// The tile shape is 3mm_opt's (same as 2mm_opt's): TS = TK = 16. Each
...
// `SYCLRewrite3mmRange` (-fsycl-rewrite-3mm-range) rewrites ALL THREE
// range<2> launches from (N,N) to (ceil16(N), ceil16(N)) -- the FULL padded
// grid, one work-item per output CELL -- and the SYCL runtime env var
// SYCL_FORCE_LOCAL_SIZE=16,16 (this tree) makes the launch carry the
// explicit local size (16,16). With local (16,16) guaranteed, all of
// 3mm_opt's index math reduces to arithmetic on the item id: ty = irow % 16,
// tx = jcol % 16 -- no local_id/group_id ops needed.
//
// TWO rewrite sites are required per kernel (6 total), because the cgeist
// frontend emits each body in TWO places with different consumers:
//
//   1. `func.func @_ZZZN..._clESB_` (top-level, signature
//      (memref<?x!llvm.struct<(i64, out_acc, r_acc, r_acc)>>, !sycl_item_2_)):
//      the wrapper path (`gpu.func @__pf_kernel_wrapperI15Polybench_3mm_NE`
...
//
//   2. `gpu.func @_ZTS15Polybench_3mm_{1,2,3}` (the direct kernels, bodies
//      born INLINED from the frontend -- the path that always runs here).
//      The entry-block prologue (kernel-arg -> captured struct reassembly +
...
//    bug in 3mm_opt itself, which is never run at N % 16 != 0). Instead the
...
//
// No reqd_work_group_size is attached (unusable on this stack: UR HIP stubs
// COMPILE_WORK_GROUP_SIZE to {0,0,0} and the runtime rejects 3-element reqd
// against a 2-D launch); the (16,16) work-group shape is supplied at launch
// time by SYCL_FORCE_LOCAL_SIZE.
//
// Two gates (same as the sibling passes): (1) opt-in flag
// `--sycl-3mm-local-tile` (off; wired in cgeist driver.cc), and (2) the
...
namespace mlir {
namespace sycl {
#define GEN_PASS_DEF_THREEMMLOCALTILEPASS
#include "mlir/Dialect/SYCL/Transforms/Passes.h.inc"
} // namespace sycl
} // namespace mlir

#define DEBUG_TYPE "sycl-3mm-local-tile"
...
/// The mangled kernel name fragments uniquely identifying the three 3mm body
/// functions. 3mm.cpp's kernel tag classes are `Polybench_3mm_1` (E += A*B),
/// `Polybench_3mm_2` (F += C*D) and `Polybench_3mm_3` (G += E*F) -- all three
/// read_write outputs, all three seeded from the existing OUT value.
static constexpr llvm::StringRef THREE_MM_KERNEL_1_FRAGMENT =
    "Polybench_3mm_1";
static constexpr llvm::StringRef THREE_MM_KERNEL_2_FRAGMENT =
    "Polybench_3mm_2";
static constexpr llvm::StringRef THREE_MM_KERNEL_3_FRAGMENT =
    "Polybench_3mm_3";
...
static constexpr llvm::StringRef THREE_MM_DIRECT_KERNEL_1 =
    "_ZTS15Polybench_3mm_1";
static constexpr llvm::StringRef THREE_MM_DIRECT_KERNEL_2 =
    "_ZTS15Polybench_3mm_2";
static constexpr llvm::StringRef THREE_MM_DIRECT_KERNEL_3 =
    "_ZTS15Polybench_3mm_3";

/// Tile shape, matching polybench/orise/3mm_opt.cpp: TS is the work-group
...
/// AMDGPULowerModuleLDS from restructuring a shared global across the three
/// kernels).
static constexpr llvm::StringRef TILE_A_KERNEL_1 = "_3mm_k1_aTile";
static constexpr llvm::StringRef TILE_B_KERNEL_1 = "_3mm_k1_bTile";
static constexpr llvm::StringRef TILE_A_KERNEL_2 = "_3mm_k2_aTile";
static constexpr llvm::StringRef TILE_B_KERNEL_2 = "_3mm_k2_bTile";
static constexpr llvm::StringRef TILE_A_KERNEL_3 = "_3mm_k3_aTile";
static constexpr llvm::StringRef TILE_B_KERNEL_3 = "_3mm_k3_bTile";

/// Number of members in each 3mm lambda's captured struct:
/// (i64 size_, OUT accessor, IN1 read, IN2 read).
static constexpr size_t THREE_MM_STRUCT_MEMBERS = 4;

/// How the scalar accumulator is seeded before the k loop. All three 3mm
/// kernels accumulate into the existing output (read_write; the baseline
/// bodies are all `X[item] += ...`), so every kernel uses SeedFromC.
enum class SeedMode { SeedFromC, ZeroSeed };

class ThreeMmLocalTilePass
    : public mlir::sycl::impl::ThreeMmLocalTilePassBase<ThreeMmLocalTilePass> {
public:
...
private:
  ...
  /// Returns the seed mode if \p func is one of the three 3mm lambda bodies
  /// (matched on the `sycl.kernel_func_obj` anchor), nullopt otherwise.
  std::optional<SeedMode> get3mmSeedMode(func::FuncOp func) const {
    ...
    for (FlatSymbolRefAttr symbol :
      ...
      if (symbol.getValue().contains(THREE_MM_KERNEL_1_FRAGMENT) ||
          symbol.getValue().contains(THREE_MM_KERNEL_2_FRAGMENT) ||
          symbol.getValue().contains(THREE_MM_KERNEL_3_FRAGMENT))
  ...
  /// `gpu.func @_ZTS15Polybench_3mm_{1,2,3}` to the LDS-tile form. Keeps the
...
/// exactly the 3mm capture layout (i64 size_ first, then the 3 accessors --
/// the OUT accessor is rw for all three kernels; both rw and dw shapes are
/// accepted here, the seed mode is decided by the kernel anchor).
static bool is3mmStruct(Type elemTy) {
  auto sTy = dyn_cast<LLVM::LLVMStructType>(elemTy);
  if (!sTy || sTy.getBody().size() != THREE_MM_STRUCT_MEMBERS)
    return false;
...
bool ThreeMmLocalTilePass::rewriteBodyTile(func::FuncOp body,
                                           SeedMode seedMode) {
  ...
  if (!arg0MT || !is3mmStruct(arg0MT.getElementType()))
    return false;
...
bool ThreeMmLocalTilePass::rewriteDirectKernel(gpu::GPUFuncOp kernel,
                                               SeedMode seedMode) {
  ...
  for (Operation &op : entry.getOperations()) {
    if (auto p2m = dyn_cast<polygeist::Pointer2MemrefOp>(op)) {
      ...
      if (mt && !structDef) {
        if (is3mmStruct(mt.getElementType())) {
...
bool ThreeMmLocalTilePass::emitTileBody(OpBuilder &b,
  ...
  // Per-kernel tile names (so AMDGPULowerModuleLDS never restructures a
  // shared global across the three kernels): resolve the kernel identity
  // from the enclosing function -- the wrapper-site func.func carries the
  // `sycl.kernel_func_obj` anchor; the direct-site gpu.func's own name IS
  // the kernel name. Defaults to the kernel-1 pair (only reachable if
  // neither anchor matches, in which case the body is not a 3mm kernel and
  // the rewrite is harmless).
  llvm::StringRef aTileName = TILE_A_KERNEL_1;
  llvm::StringRef bTileName = TILE_B_KERNEL_1;
  Operation *anchorOp = structMemref.getParentRegion()->getParentOp();
  while (anchorOp && !isa<func::FuncOp, gpu::GPUFuncOp>(anchorOp))
    anchorOp = anchorOp->getParentOp();
  if (auto f = dyn_cast_or_null<func::FuncOp>(anchorOp)) {
    if (auto kernelFuncObj = f->getAttrOfType<ArrayAttr>(
            sycl::SYCLDialect::getKernelFuncObjAttrName())) {
      for (FlatSymbolRefAttr symbol :
           kernelFuncObj.getAsRange<FlatSymbolRefAttr>()) {
        if (symbol.getValue().contains(THREE_MM_KERNEL_2_FRAGMENT)) {
          aTileName = TILE_A_KERNEL_2;
          bTileName = TILE_B_KERNEL_2;
        } else if (symbol.getValue().contains(THREE_MM_KERNEL_3_FRAGMENT)) {
          aTileName = TILE_A_KERNEL_3;
          bTileName = TILE_B_KERNEL_3;
        }
      }
    }
  } else if (auto k = dyn_cast_or_null<gpu::GPUFuncOp>(anchorOp)) {
    if (k.getName().contains(THREE_MM_KERNEL_2_FRAGMENT)) {
      aTileName = TILE_A_KERNEL_2;
      bTileName = TILE_B_KERNEL_2;
    } else if (k.getName().contains(THREE_MM_KERNEL_3_FRAGMENT)) {
      aTileName = TILE_A_KERNEL_3;
      bTileName = TILE_B_KERNEL_3;
    }
  }
  ...
  // Verified from the cgeist IR dump of the 2mm sibling (identical capture
  // layout): member 2 is subscripted at (i,k) and member 3 at (k,j) in all
  // baseline bodies.
  ...
  // --- seed: ALL THREE kernels are SeedFromC -- read the existing OUT at
  // (irow, jcol) (every 3mm baseline body is `X[item] += ...` against a
  // read_write accessor). ZeroSeed is kept for symmetry with the 2mm pass.
  Value acc0;
  ...
  scf::ForOp ktLoop = b.create<scf::ForOp>(
      ...
      [&](OpBuilder &kb, Location kl, Value kt, ValueRange accArgs) {
        ...
        // global columns -> coalesced (3mm_opt's submitTiledGemm).
        //
        ...
        // a latent bug in 3mm_opt itself (never run at N % 16 != 0).
...
void ThreeMmLocalTilePass::runOnOperation() {
  ModuleOp module = getOperation();

  if (!targetsAmdgcnSyclmlir(module)) {
    LLVM_DEBUG(llvm::dbgs() << "ThreeMmLocalTile: target is not "
  ...
  // Rewrite ALL THREE 3mm lambda bodies to the LDS-tile form (each work-item
  ...
  // of ALL THREE launches) is the LLVM pass SYCLRewrite3mmRange
  // (-fsycl-rewrite-3mm-range); the (16,16) work-group shape is supplied at
  ...
  // never trigger).
  SmallVector<func::FuncOp, 4> threeMmBodies;
  module.walk([&](func::FuncOp func) {
    if (get3mmSeedMode(func))
      threeMmBodies.push_back(func);
  });

  for (func::FuncOp body : threeMmBodies) {
    ++NumDetected;
    LLVM_DEBUG(llvm::dbgs() << "ThreeMmLocalTile: detected body " << body.getName()
                            << " (" << body.getNumArguments() << " args)\n");
    if (rewriteBodyTile(body, *get3mmSeedMode(body)))
      ++NumRewritten;
  }

  // Rewrite site 2 -- the DIRECT kernels
  // gpu.func @_ZTS15Polybench_3mm_{1,2,3} (bodies born inlined from the
  // frontend; the path that always runs). Without this, the host pass pads
  // the launch while the direct kernel still runs the baseline single-cell
  // body -- an entirely untiled kernel.
  SmallVector<gpu::GPUFuncOp, 4> directKernels;
  module.walk([&](gpu::GPUFuncOp kernel) {
    if (kernel.getName() == THREE_MM_DIRECT_KERNEL_1 ||
        kernel.getName() == THREE_MM_DIRECT_KERNEL_2 ||
        kernel.getName() == THREE_MM_DIRECT_KERNEL_3)
      directKernels.push_back(kernel);
  });

  for (gpu::GPUFuncOp kernel : directKernels) {
    ++NumDetected;
    LLVM_DEBUG(llvm::dbgs() << "ThreeMmLocalTile: detected direct kernel "
                            << kernel.getName() << "\n");
    if (rewriteDirectKernel(kernel, SeedMode::SeedFromC))
      ++NumRewritten;
  }
}

std::unique_ptr<Pass> mlir::sycl::createThreeMmLocalTilePass() {
  return std::make_unique<ThreeMmLocalTilePass>();
}
