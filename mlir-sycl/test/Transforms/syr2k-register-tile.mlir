// RUN: sycl-mlir-opt %s -split-input-file -sycl-syr2k-register-tile \
// RUN:     -mlir-pass-statistics 2>&1 | FileCheck %s

// COM: Syr2kRegisterTilePass lit coverage.
// COM:
// COM: The pass rewrites TWO sites: (1) this top-level lambda-body `func.func`
// COM: (the WRAPPER path: `gpu.func @__pf_kernel_wrapperI6Syr2k1EE` calls it
...
// COM: kernel `gpu.func @_ZTS6Syr2k1` whose body is born INLINED from the
...
// COM: targets the real Syr2k1 lambda body, whose captured-struct argument is
// COM: typed
// COM:   `memref<?x!llvm.struct<(i64, !sycl_accessor_2_f32_rw_dev,
// COM:                            !sycl_accessor_2_f32_r_dev,
// COM:                            !sycl_accessor_2_f32_r_dev)>>`
// COM: (sycl.kernel_func_obj = [@_ZTS6Syr2k1, ...] anchor; FOUR members --
// COM: M_ bound, C read_write, A read, B read). sycl-mlir-opt's asm parser
// COM: rejects `memref<?x!llvm.struct<...>>` as a memref element type
// COM: ("invalid memref element type") -- a parser limitation, not a pass
...
// COM: a fail-safe `bi < M && bj < M` guard, scf.for k with 16 loads feeding
// COM: 2 math.fma per cell -- the A*B^T and B*A^T terms -- clamped loads,
// COM: predicated stores, NO reqd_work_group_size, no scf.while/stack-k/LDS/
// COM: barrier) is verified out-of-tree via the cgeist IR dump
// COM: (`-Xcgeist --sycl-syr2k-register-tile`); the paired host-side launch
// COM: shrink is the LLVM pass SYCLRewriteSyr2kRange
// COM: (-fsycl-rewrite-syr2k-range).
// COM:
// COM: What this lit test DOES lock in (all parseable): the Syr2k1 detection
...
// COM: Syr2k1 anchor is detected (num-detected = 1); the non-struct arg0 trips
...
module attributes {llvm.target_triple = "amdgcn-amd-amdhsa-syclmlir"} {
  func.func private @syr2k_body(%arg0: memref<?xi64>, %arg1: !sycl_item_2_) attributes {sycl.kernel_func_obj = [@_ZTS6Syr2k1]} {
    return
  }
}
...
// CHECK-LABEL: func.func private @syr2k_body
