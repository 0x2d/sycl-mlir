...
#include "llvm/Support/Extension.def"
  ...
  if (!CodeGenOpts.DisableLLVMPasses) {
    ...
    // Host-side launch-range rewrite for the syr2k benchmark kernel, paired
    // with the device-side MLIR pass `sycl-syr2k-register-tile` (cgeist).
    // Runs at PipelineStartEP for the same reason as the syrk hook above.
    if (LangOpts.SYCLIsHost && LangOpts.SYCLRewriteSyr2kRange)
      PB.registerPipelineStartEPCallback(
          [&](ModulePassManager &MPM, OptimizationLevel Level) {
            MPM.addPass(SYCLRewriteSyr2kRangePass());
          });

    // Host-side launch-range rewrite for BOTH 2mm benchmark kernels, paired
    // with the device-side MLIR pass `sycl-2mm-local-tile` (cgeist).
    // Runs at PipelineStartEP for the same reason as the syrk hook above.
    if (LangOpts.SYCLIsHost && LangOpts.SYCLRewrite2mmRange)
      PB.registerPipelineStartEPCallback(
          [&](ModulePassManager &MPM, OptimizationLevel Level) {
            MPM.addPass(SYCLRewrite2mmRangePass());
          });

    // Host-side launch-range rewrite for ALL THREE 3mm benchmark kernels,
    // paired with the device-side MLIR pass `sycl-3mm-local-tile` (cgeist).
    // Runs at PipelineStartEP for the same reason as the syrk hook above.
    if (LangOpts.SYCLIsHost && LangOpts.SYCLRewrite3mmRange)
      PB.registerPipelineStartEPCallback(
          [&](ModulePassManager &MPM, OptimizationLevel Level) {
